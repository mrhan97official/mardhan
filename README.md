...
Kartu **CPU** menampilkan pemakaian CPU instans Go sejak sampel sebelumnya (sampel pertama dirata-ratakan sejak instans hidup); **Memory** menampilkan besar heap Go instans tersebut dalam MiB. Karena API berjalan di Vercel Functions, dua angka ini bukan penggunaan semua mesin atau seluruh deployment. Grafik CPU/Memory mengumpulkan sampel nyata tiap 10 detik saat halaman terlihat, menunggu sedikitnya dua sampel, dan tidak menambah titik baru saat data gagal diperbarui. Garis tetap datar jika nilai terukur tidak berubah.
