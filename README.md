...
Untuk **Network** (byte respons HTTP dalam MiB) dan **Requests** (jumlah request), buka **Settings → Zona Cloudflare**, pilih zona dari akun `CF_ACCOUNT_ID`, lalu tekan **Setujui zona dan simpan ke Vercel**. `CF_API_TOKEN` membutuhkan **Zone:Zone:Read** agar zona ditemukan dan **Account Analytics Read** agar metriknya dapat dibaca. Aplikasi memverifikasi analitik, menyimpan pilihan yang disetujui di D1 agar langsung aktif, lalu menulis `CF_ZONE_ID` ke environment project Vercel saat ini dengan `VERCEL_TOKEN` untuk deployment berikutnya. Identitas project diambil dari variabel sistem `VERCEL_PROJECT_ID`, atau dicari dari domain produksi/link Git bila variabel sistem tidak tersedia; jika ambigu, aktifkan System Environment Variables pada project Vercel. Jika penulisan Vercel gagal, metrik tetap aktif dari D1 dan Settings menampilkan status belum tersinkron serta tombol untuk mencoba lagi. Anda tetap dapat mengisi `CF_ZONE_ID` manual sebagai fallback sebelum ada zona yang disetujui. Pastikan trafik domain benar-benar melalui proxy Cloudflare; domain vercel.app sendiri tidak memiliki zona Cloudflare milik Anda. Data GraphQL dijumlah selama 24 jam terakhir untuk **semua hostname** dalam zona, dengan grafik per jam dan pembaruan paling cepat tiap menit. Cloudflare dapat memakai sampling adaptif sehingga total trafiknya berupa estimasi. Jika zona, izin, atau layanan analitik tidak tersedia, kartu menampilkan tanda `—` beserta petunjuk konfigurasi; tabel D1 `infra_metrics` dan data contoh tidak digunakan sebagai metrik langsung.
...
Waktu mulai dan selesai tahap juga dapat dibaca dari data D1 lama yang menyimpan detik Unix sebagai angka desimal berakhiran `.0`; pembaruan tahap baru disimpan sebagai bilangan bulat. Dropdown lonceng membaca status pipeline yang sama dan hanya menampilkan proses yang masih ada dalam periode riwayat lima menit tersebut.
