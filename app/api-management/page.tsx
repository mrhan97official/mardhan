...
export default function ApiManagementPage() {
  ...
  return (
    <AppShell title="API Management" subtitle="Endpoint, API key baca, dan uji respons nyata">
      {error && <p role="alert" className="rounded-xl bg-red-500/10 p-2 text-sm text-red-300">{error} {!snapshot && <Link className="underline" href="/databases">Periksa Databases</Link>}</p>}
      {message && <p role="status" className="rounded-xl bg-emerald-500/10 p-2 text-sm text-emerald-300">{message}</p>}
      ...
      {snapshot && <>
        ...
        <section className="card space-y-2 p-2">
          ...
          <form onSubmit={submitAPI} className="grid gap-2 sm:grid-cols-2 md:grid-cols-5 md:gap-2">
          ...
          <div className="grid gap-2 md:grid-cols-2">{snapshot.apis.map((api) => {
            ...
            return <div key={api.id} className="rounded-xl border border-base-border bg-base-850 p-2">
              ...
              <p className="mt-2 text-xs text-slate-400">{check ? `Uji terakhir: ${check.status_code || "gagal"} · ${check.latency_ms} ms · ${check.checked_at}` : "Belum diuji"}</p>
              <div className="mt-2 flex gap-2"><button type="button" disabled={busy || !api.enabled} onClick={() => void run("test_api", { id: api.id })} className="rounded-lg border border-base-border px-3 py-2 text-xs disabled:opacity-50">Uji sekarang</button>
        ...
        <section className="card space-y-2 p-2">
          ...
          <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3">{snapshot.scopes.map((scope) => <div key={scope} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-base-border bg-base-850 px-2 py-2 text-xs">
        ...
        <section className="card space-y-2 p-2">
          ...
          <form onSubmit={submitKey} className="space-y-2"><input required minLength={2} maxLength={80} value={keyName} onChange={(e) => setKeyName(e.target.value)} className={field} placeholder="Nama klien, mis. dashboard tim" />
            <div className="flex flex-wrap gap-2">{snapshot.scopes.map((scope) => <label key={scope} className="flex items-center gap-2 rounded-lg border border-base-border px-2 py-2 text-xs">
          ...
          {newKey && <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-2"><p className="text-xs text-amber-300">Salin sekali lalu simpan di tempat aman:</p>
          ...
          <div className="space-y-2">{snapshot.keys.map((key) => <div key={key.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-base-border p-2 text-xs">
        ...
        <section className="card p-2"><h2 className="text-base font-bold">Riwayat tindakan admin</h2>
          <div className="mt-2 space-y-2 text-xs text-slate-400">{snapshot.audit.map((item, index) => <p key={`${item.created_at}-${index}`}>{item.created_at} · {item.action} · {item.target}</p>)}
